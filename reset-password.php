...
border: 1px solid rgba(255,255,255,0.25);'>
        ...
        <center>
          <img src="assets/images/logo/logo-hoolkar.png"  height='100' alt="" style='padding: 5% 4%;'>
        </center>
        ...
        <p> Please enter your email to recieve reset password instructions.</p>
          <div class="form-label-group">
            <input type="email" id="username" name='username' required class="form-control" placeholder="Email Address" autofocus="" autocomplete="off"> <label for="username">Email Address</label>
          </div>
        ...
        <div class="text-center pt-3">
         I remember my password. <a href="login" class="link">Login</a>
        </div><!-- /recovery links -->
      </form><!-- /.auth-form -->
      <?php ?>
      <!-- copyright -->
      <footer class="auth-footer"> © 2022 All Rights Reserved. Developed by <a href="#"><strong>Yakub Ahmed</strong></a>
      </footer>
    </main><!-- /.auth -->
    <!-- BEGIN BASE JS -->
  ...
  $(document).ready(function(){
    ...
    $("#frmResetPassword").on('submit', function(e){
      ...
      var count = 0;
           ...
           $.ajax({
               url: 'https://app.submalco.so/jquery/forgot-password.php', 
               type:'post', 
               ...
               success: function(res){
                   if(res == 'success'){
                    ...
                    $('.msgContent').html("We have sent reset password instruction on your email.")
                   }else if (res == 'not-found'){

                      $('#errMsg').removeClass('d-none')
                      $('.msgContent').html("The email address that you have entered is not found on our database.")
                      $('#loading-login').addClass('d-none')
    ...
    $('#frmAuth').on('submit', function(e){
      ...
      $.ajax({
          url: 'https://app.submalco.so/jquery/login.php', 
          type:'post', 
          data: $('#frmAuth').serialize(),
          success: function(res){
            if(res == "success"){
              location.href = 'https://app.submalco.so/'
